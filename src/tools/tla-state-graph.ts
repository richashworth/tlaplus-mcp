  ...
  server.tool(
    ...
    "Load a TLC-generated DOT state graph file and return it in a structured format for exploration. Supports raw DOT, simplified adjacency list, or full JSON format with disambiguated actions, invariants, and violation traces.",
    {
      dot_file: absolutePath
        .optional()
        .describe(
          "Absolute path to the .dot state graph file generated by TLC (not needed in traces_only mode)",
        ),
      cfg_file: absolutePath
        .optional()
        .describe(
          "Absolute path to the .cfg file (for invariant/property names)",
        ),
      tlc_output_file: absolutePath
        .optional()
        .describe(
          "Absolute path to the TLC output file (for violation traces)",
        ),
      tlc_output: z
        .string()
        .optional()
        .describe(
          "Raw TLC output string (for violation traces). If both tlc_output and tlc_output_file are provided, tlc_output takes precedence.",
        ),
      format: z
        .enum(["dot", "structured", "json"])
        .default("json")
        .describe(
          "Output format: 'dot' (raw), 'structured' (adjacency list), or 'json' (full structured output with states, transitions, violations, happy paths)",
        ),
      traces_only: z
        .boolean()
        .default(false)
        .describe(
          "When true, build a minimal graph from TLC output traces alone (no DOT file needed). Returns partial: true.",
        ),
      output_file: absolutePath
        .optional()
        .describe(
          "Write JSON to this file instead of returning it inline (json format only). Response will contain a compact summary.",
        ),
    },
    async ({
      dot_file,
      ...
      try {
        ...
        if (traces_only) {
          ...
          const tlcOutputStr =
            tlc_output ??
            (tlc_output_file
              ? (validateFileExists(tlc_output_file, "TLC output file"),
                fs.readFileSync(tlc_output_file, "utf-8"))
              : undefined);
          if (!tlcOutputStr) {
            throw new Error(
              "traces_only requires tlc_output or tlc_output_file",
            );
          }

          const traceGraph = buildGraphFromTraces(tlcOutputStr);
          const transitions = disambiguateActions(
            traceGraph.states,
            traceGraph.edges,
          );
          ...
          const violationFinalStates = buildViolationFinalStates(
            traceGraph.violations,
          );
          let happyPaths = discoverHappyPaths(
            traceGraph.initialStateId,
            transitions,
            violationFinalStates,
          );

          // Fallback: extract non-violating prefixes from violation traces
          if (happyPaths.length === 0 && traceGraph.violations.length > 0) {
            happyPaths = extractPrefixHappyPaths(
              traceGraph.violations,
              violationFinalStates,
            );
          }
        ...
        const happyPaths = discoverHappyPaths(
...
interface StateGraphData {
  ...
  transitions: Record<
    string,
    import("../parsers/action-disambiguator.js").DisambiguatedTransition[]
  >;
  invariants: string[];
